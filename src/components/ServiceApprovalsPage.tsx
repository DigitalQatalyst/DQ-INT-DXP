...
const summaryData = [{
  ...
  count: mockServices.filter(s => s.status === 'Pending').length,
  icon: ClockIcon,
  ...
  count: mockServices.filter(s => s.status === 'Step 2 Pending').length,
  icon: InfoIcon,
  ...
  count: mockServices.filter(s => s.status === 'Approved').length,
  ...
  id: 'rejected',
  title: 'Rejected',
  count: mockServices.filter(s => s.status === 'Rejected').length,
  icon: XCircleIcon,
...
export const ServiceApprovalsPage: React.FC = () => {
  ...
  const handleApproveService = (serviceId: string) => {
    setSelectedService(mockServices.find(item => item.id === serviceId));
  ...
  const uniquePartners = Array.from(new Set(mockServices.map(service => service.partner)));
